# AI Moderator Commercial License Terms

Copyright (c) 2026 Alexandr Spivak. All rights reserved.

These terms are a baseline commercial license template for paid AI Moderator
customers. They apply only when the copyright holder grants a written commercial
license, invoice, order form, contract, or other written permission that
references this file or clearly grants commercial rights.

## 1. License Grant

After purchase and subject to the paid agreement, the customer receives a
non-exclusive, non-transferable license to use, run, modify, and integrate the
Software for the licensed project, product, organization, model, or deployment
scope.

## 2. Permitted Use

Unless the paid agreement says otherwise, the customer may:

- run the Software in private, internal, production, or hosted environments;
- modify the Software for their own licensed use;
- integrate the Software into Discord bots, Telegram bots, moderation services,
  dashboards, APIs, and related products;
- run the trained model artifacts for the licensed deployment;
- create private forks and private derivative works for the licensed scope.

## 3. Attribution And Authorship

The customer must not remove, hide, or alter copyright notices, license text,
authorship notices, or attribution information from source code copies.

The customer must not claim authorship of the original Software. The original
Software remains authored by Alexandr Spivak.

## 4. Restrictions

Unless the paid agreement expressly allows it, the customer may not:

- publish or distribute the original source code;
- sell, sublicense, lease, rent, or share the source code as a standalone
  product;
- make the repository public;
- provide the source code to third parties outside the licensed organization;
- redistribute trained model artifacts outside the licensed scope;
- remove copyright or attribution notices;
- use the Software outside the purchased license scope.

## 5. Ownership

The original Software remains owned by Alexandr Spivak.

Customer modifications created under a paid license may be owned by the customer
unless the paid agreement states otherwise, but those modifications do not remove
the author's ownership of the original Software.

## 6. White-Label, Resale, And SaaS Rights

White-label distribution, resale, sublicensing, marketplace distribution, public
SaaS use, or source-code redistribution require explicit written permission in
the paid agreement.

## 7. No Warranty

THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR
IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY,
FITNESS FOR A PARTICULAR PURPOSE, NON-INFRINGEMENT, SECURITY, RELIABILITY, OR
AVAILABILITY.

## 8. Conflict

If a signed contract, invoice, order form, or written agreement conflicts with
this file, the signed or expressly approved agreement controls.
