        ...
        try:
            with warnings.catch_warnings():
                ...
                with Image.open(BytesIO(downloaded.content)) as image:
                    ...
                    if downloaded.declared_content_type.casefold() != detected_mime:
                        raise UnsupportedMediaError("declared and detected MIME types do not match")
                    if getattr(image, "n_frames", 1) != 1 or getattr(image, "is_animated", False):
                        raise UnsupportedMediaError("animated or multi-frame images are not supported")
                    ...
                    image.load()
                    normalized_bytes = self._strip_metadata(image, detected_mime)
                    fingerprint_luma = image.convert("L").resize((32, 32), Image.Resampling.LANCZOS).tobytes()
