...
class ModerationFeedbackRequestSchema(ApiModel):
    ...
    message_id: str | None = Field(
        default=None, min_length=1, max_length=64, pattern=r"^[0-9A-Za-z_-]+$"
    )
    ...
    original_action: ModerationAction | None = None
    moderator_id: str | None = Field(
        default=None, max_length=128, pattern=r"^[a-f0-9]{64}$"
    )
    ...
    def _require_reference(self) -> "ModerationFeedbackRequestSchema":
        ...
        if self.event_id is None and self.guild_id is None:
            raise ValueError("guild_id is required when message_id is used")
        return self
